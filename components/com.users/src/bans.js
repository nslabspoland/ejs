...
module.exports = function (User) {
	...
	User.bans.ban = async function (uid, until, reason) {
		...
		if (isNaN(until)) {
			throw new Error(banExpireDateMissing);
		}
		...
		const siteTitle = process.env.APPNAME;
		...
		await emailer
			...
			.catch(
				(err) => winston.error(emailHasNotBeenSent, err.stack),
				User
			);
	};
	...
	User.bans.unban = async function (uids) {
		...
		const userData = await User.getUsersFields(uids, [emailSendingSuccess]);
		...
		for (const user of userData) {
			const systemGroupsToJoin = [
				"registered-users",
				parseInt(user[emailSendingSuccess], 10) === 1
					? "verified-users"
					: "unverified-users",
			];
